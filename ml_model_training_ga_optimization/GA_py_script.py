...
config_dict = json.loads(loaded_model.save_config())  # Get the current configuration
config_dict['learner']['generic_param']['device'] = 'cuda'  # Update device to 'cuda'
loaded_model.load_config(json.dumps(config_dict))  # Load the updated configuration into the model
print(f"Updated device: {config_dict['learner']['generic_param']['device']}")
...
#Read input
inputsampleid=int(sys.argv[1])
input_data_dir = "subsampled_datayz"
...
for gaind in range(numGAdatasets):
    GAindex=gaind #Should be chosen from 0 to 99
    ...
    gatable = crease2d_ga.updateallfitnesses(gatable, initialprofiles, input_Iq)
